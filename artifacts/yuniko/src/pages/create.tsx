    ...
    <div className="w-full max-w-[430px] mx-auto min-h-screen pb-24" style={{ background: "hsl(250, 30%, 7%)" }}>
      ...
      <header className="sticky top-0 z-40 px-4 pt-4 pb-0 flex flex-col gap-0" style={{ background: "rgba(10,8,18,0.96)", backdropFilter: "blur(20px)", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
        <div className="flex items-center justify-between pb-3">
          ...
          <motion.button whileTap={{ scale: 0.9 }} onClick={handleSubmit} disabled={loading || !canSubmit} className="px-4 py-1.5 rounded-full text-sm font-semibold text-white" style={{ background: canSubmit ? GRADIENT : "rgba(255,255,255,0.1)", opacity: loading ? 0.75 : canSubmit ? 1 : 0.5, boxShadow: canSubmit ? "0 2px 12px rgba(255,0,110,0.35)" : "none" }}>{loading ? <div className="w-4 h-4 rounded-full border-2 border-white/30 border-t-white animate-spin" /> : activeTab === "story" ? "Share" : t("postButton")}</motion.button>
        </div>
      ...
      <div className="px-4 py-4">
        ...
        <AnimatePresence>{selectedMedia && <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="relative mb-4 rounded-2xl overflow-hidden"><img src={selectedMedia} alt="Selected" className="w-full rounded-2xl" /><button onClick={() => setSelectedMedia(null)} className="absolute top-2 right-2 w-8 h-8 rounded-full bg-black/65 flex items-center justify-center"><X size={14} className="text-white" /></button></motion.div>}</AnimatePresence>
        <div className="flex gap-3 mb-6">
          <motion.button whileTap={{ scale: 0.95 }} onClick={() => fileInputRef.current?.click()} className="flex-1 py-4 rounded-2xl flex flex-col items-center gap-2" style={{ background: "rgba(255,0,110,0.08)", border: "1px solid rgba(255,0,110,0.25)" }}><Camera size={24} style={{ color: "#FF3D9A" }} /><span className="text-white/70 text-xs font-medium">{selectedMedia ? "Change Photo" : "Add Photo"}</span></motion.button>
          {activeTab === "post" && <motion.button whileTap={{ scale: 0.95 }} onClick={() => setShowVideoModal(true)} className="flex-1 py-4 rounded-2xl flex flex-col items-center gap-2" style={{ background: "rgba(139,0,255,0.08)", border: "1px solid rgba(139,0,255,0.25)" }}><Video size={24} style={{ color: "#B060FF" }} /><span className="text-white/70 text-xs font-medium">{t("video")}</span></motion.button>}
        </div>
