...
router.get("/blocked-users", authMiddleware, async (req,res)=>{
 ...
 try { const rows=await selectRows("blocked_users",{filters:[eq("userId",userId)]});
...
router.delete("/blocked-users/:id",authMiddleware,async(req,res)=>{
 ...
 try{await deleteRows("blocked_users",[eq("userId",userId),eq("blockedUserId",blockedUserId)]);return res.json({success:true})}catch(err){return supabaseError(res,err)}
});
router.post("/blocked-users/:id",authMiddleware,async(req,res)=>{
 ...
 try{await insertRow("blocked_users",{userId,blockedUserId});return res.status(201).json({success:true})}catch(err){return supabaseError(res,err)}
});
